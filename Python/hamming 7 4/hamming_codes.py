...
#Credit: https://colindcarroll.wordpress.com/2012/12/11/hamming-codes-in-python/
...
class Message:
  ...
  def corruptMessage(self,nErrors):
    ...
    indices = np.asarray(random.sample(range(0, self.encodedMessage.size), nErrors))
    #print ''
    #print self.message
    #print self.encodedMessage
    #print indices, len(set(indices))
    #tmp = np.random.randint(0,self.encodedMessage.size,nErrors)
    #print tmp, len(set(tmp))
    #  np.random.randint(0,self.encodedMessage.size,nErrors)
    #ema ema print 'nErrors', nErrors, 'MessageLength:', self.encodedMessage.size
    #print type(indices)
    #print type(np.random.randint(0,self.encodedMessage.size,nErrors))
    noise[indices] = 1
